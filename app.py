...
if not price_raw.empty:
    ...
    if not inst_raw.empty:
        inst_sum = inst_raw.tail(9) # 三大法人各三日
    ...
    with tab_holder:
        if not big_holders.empty:
            ...
            fig_h.update_layout(height=400, template="plotly_dark", title="近 12 週千張大戶持股比例趨勢")
            st.plotly_chart(fig_h, use_container_width=True)
        else:
            st.info("該標的暫無大戶籌碼週資料。")

else:
    st.error("❌ 無法讀取資料。可能原因：1. Token 錯誤 2. 網路連線問題 3. 該股票尚未上市。")

st.divider()
st.caption(f"數據更新時間：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | 數據源：FinMind API")