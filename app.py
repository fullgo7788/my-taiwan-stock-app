...
def safe_get_data(dataset, data_id=None, start_date=None):
    ...
    try:
        ...
        # 只在偵錯時顯示，或使用 st.sidebar.warning
        pass
    return pd.DataFrame()
...
if not price_raw.empty:
    ...
    if vol > 4.5:
        tag, tag_color, desc = "⚡ 短線爆發型", "orange", "波幅劇烈，建議快進快出。"
    elif strength > 0.8:
        tag, tag_color, desc = "📈 長線趨勢型", "lime", "趨勢穩健，適合沿月線布局。"
    else:
        tag, tag_color, desc = "🌀 區間震盪型", "cyan", "盤整蓄勢中，建議高拋低吸。"
    ...
    st.markdown(f"""
        <div style="background-color: #1e1e1e; padding: 20px; border-radius: 10px; border-left: 10px solid {tag_color};">
            ...
            <p style="margin:5px 0 0 0; color: #dcdcdc;">{desc}</p>
        </div>
    ...
    # 大戶與法人處理
    big_holders = holder_raw[holder_raw['hold_class'] == '1000以上'].sort_values('date') if not holder_raw.empty else pd.DataFrame()
    ...
    st.error("⚠️ 無法取得數據。請檢查 Token 是否過期或超過 API 流量限制。")